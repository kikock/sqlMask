...
//        log.info("start: " + node.getParserPosition().getColumnNum() + ";end: " + node.getParserPosition().getEndColumnNum());
        if (node instanceof SqlIdentifier) {
            ...
            if (operator instanceof SqlAsOperator) {
                getOriginColumn(((SqlBasicCall) node).getOperandList().get(0), context,
                        sqlNodeAndOriginColumnStringMap);
                ...
                for (int i = 0; i < ((SqlBasicCall) node).getOperandList().size(); i++) {
                    getOriginColumn(((SqlBasicCall) node).getOperandList().get(i), context,
                            sqlNodeAndOriginColumnStringMap);
                }
            }
            //后面的不需要了，留着防万一
