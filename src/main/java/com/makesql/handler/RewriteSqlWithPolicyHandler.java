...
import com.makesql.policy.MaskPolicyFactory;
import com.makesql.policy.PolicyStorage;
...
public class RewriteSqlWithPolicyHandler implements Handler {
    ...
    private void checkColumnInFunction(SqlNode node, Set<SqlParserPos> simpleColumnInFunctionCheck) {
        ...
        if (node instanceof SqlBasicCall) {
            if (((SqlBasicCall) node).getOperator() instanceof SqlFunction) {
                ...
                for (int i = 0; i < ((SqlBasicCall) node).getOperandList().size(); i++) {
                    checkColumnInFunction(((SqlBasicCall) node).getOperandList().get(i),
                            simpleColumnInFunctionCheck);
                }
            }
        }
        //        }
    ...
    private Map<String, String> getPolicy(String source) {
        PolicyStorage policyStorage = MaskPolicyFactory.getInstance("com.makesql.policy.file.PropertiesFilePolicyStorage");
